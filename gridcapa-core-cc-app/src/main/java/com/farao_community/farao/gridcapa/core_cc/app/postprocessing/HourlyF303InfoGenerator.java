...
import com.farao_community.farao.data.crac_creation.creator.fb_constraint.*;
...
class HourlyF303InfoGenerator {
    ...
    private final RaoIntegrationTask raoIntegrationTask;
    private final MinioAdapter minioAdapter;

    HourlyF303InfoGenerator(FbConstraint nativeCrac, Interval interval, RaoIntegrationTask raoIntegrationTask, MinioAdapter minioAdapter) {
        this.nativeCrac = nativeCrac;
        this.interval = interval;
        this.raoIntegrationTask = raoIntegrationTask;
        this.minioAdapter = minioAdapter;
    }

    HourlyF303Info generate() {
        HourlyRaoRequest hourlyRaoRequest = getRaoRequestForInterval(interval, raoIntegrationTask);
        HourlyRaoResult hourlyRaoResult = getRaoResultForInterval(interval, raoIntegrationTask);

        if (hourlyRaoRequest == null || hourlyRaoResult == null || !hourlyRaoResult.getStatus().equals(TaskStatus.SUCCESS)) {
    ...
    private HourlyF303Info getInfoForNonRequestedOrFailedInterval(org.threeten.extra.Interval interval, FbConstraint nativeCrac) {
    ...
    private HourlyF303Info getInfoForSuccessfulInterval(org.threeten.extra.Interval interval, HourlyRaoRequest hourlyRaoRequest, HourlyRaoResult hourlyRaoResult, FbConstraint nativeCrac) {

        Network network = getNetworkOfHourlyRequest(hourlyRaoRequest);
        FbConstraintCreationContext cracCreationContext = new FbConstraintCracCreator().createCrac(nativeCrac, network, OffsetDateTime.parse(hourlyRaoRequest.getInstant()).plusMinutes(30), CracCreationParameters.load());
    ...
    private List<CriticalBranchType> getCriticalBranchesOfSuccessfulInterval(org.threeten.extra.Interval interval, FbConstraint nativeCrac, FbConstraintCreationContext cracCreationContext, Map<State, String> statesWithCrac) {
    ...
    private List<IndependantComplexVariant> getComplexVariantsOfSuccesfulInterval(org.threeten.extra.Interval interval, FbConstraint nativeCrac, FbConstraintCreationContext cracCreationContext, RaoResult raoResult, Map<State, String> statesWithCra) {
    ...
    private Network getNetworkOfHourlyRequest(HourlyRaoRequest hourlyRaoRequest) {
        ...
        try (InputStream networkInputStream = minioAdapter.getInputStreamFromUrl(networkFileUrl)) {
            return Importers.loadNetwork(minioAdapter.getFileNameFromUrl(networkFileUrl), networkInputStream);
        } catch (IOException e) {
            throw new RaoIntegrationException(String.format("Cannot import network of hourly request of instant %s", hourlyRaoRequest.getInstant()));
        }
    }
    ...
    private RaoResult getRaoResultOfHourlyRaoResult(HourlyRaoResult hourlyRaoResult, Crac crac) {
        ...
        try (InputStream raoResultInputStream = minioAdapter.getInputStreamFromUrl(hourlyRaoResult.getRaoResultFileUrl())) {
            ...
            throw new RaoIntegrationException(String.format("Cannot import RAO result of hourly RAO response of instant %s", hourlyRaoResult.getInstant()));
        }
    }

    private static HourlyRaoRequest getRaoRequestForInterval(org.threeten.extra.Interval interval, RaoIntegrationTask raoIntegrationTask) {
        return raoIntegrationTask.getHourlyRaoRequests().stream()
                .filter(hrr -> interval.contains(java.time.Instant.parse(hrr.getInstant())))
                .findAny().orElse(null);
    }

    private static HourlyRaoResult getRaoResultForInterval(org.threeten.extra.Interval interval, RaoIntegrationTask raoIntegrationTask) {
        return raoIntegrationTask.getHourlyRaoResults().stream()
                .filter(hrr -> interval.contains(java.time.Instant.parse(hrr.getInstant())))
                .findAny().orElse(null);
    }
